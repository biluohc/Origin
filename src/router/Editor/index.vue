...
export default {
  data: function() {
    return {
      sites: deepCopy(this.store.sites),
      sites_tr_length: this.store.sites_tr_length,
      search_engines: deepCopy(this.store.search_engines),
      sites_unchange: true,
  ...
  methods: {
    ...
    save: function() {
      ...
      this.store.update(this.sites_tr_length, this.sites, this.search_engines);
      // console.log(JSON.stringify(this.store));
      this.store.save_config_to_localStorage();
      alert("通知：保存成功");
    },
    download: function() {
      var json = JSON.stringify(this.store.copy());
    ...
    sites_tr_length: {
      handler: function(val, oldVue) {
        this.sites_tr_length_unchange =
          this.sites_tr_length === this.store.sites_tr_length;
        console.log("sites_tr_length_unchange", this.sites_tr_length_unchange);
      }
    },
    sites: {
      handler: function(val, oldVue) {
        this.sites_unchange = deepEq(this.sites, this.store.sites);
